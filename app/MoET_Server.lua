	...
	--cord.new(function() 
		...
		if payload and stringStarts(payload,"1") then
			...
			if iterations >= 10 then 
				cord.new(function() lcd:writeString(string.sub("Network: " .. successes*100/count .. "." .. successes*1000/count % 10 .. "%" .. " received                     ",1,32)) end)
				iterations = 0
			end
		elseif payload and stringStarts(payload,"2") then
			...
			if iterations >= 10 then 
				cord.new(function() lcd:writeString(string.sub("Acc XYZ:        " .. value .. "                                         ",1,32)) end)
				iterations = 0
			end
